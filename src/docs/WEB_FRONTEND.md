...
| Hero | `HeroSection.tsx` + `ChessHeroRig.tsx` | **Editorial split** (Editorial-Luxury / Split-Stage): serif copy + gold CTA on the LEFT, 3D pieces clustered RIGHT — no overlap. Charcoal `#0a0a0c` (never pure black), subtle warm radial glow, eval-bar tags. The R3F Canvas is a **viewport-fixed overlay `createPortal`'d to `<body>`** (NOT a snap-container child — being a child broke the snap hook's `children[i]=section i` index math and froze scrolling). Loads `chess-hero.glb` (named King/Queen/Mic). **GSAP owns animation, R3F only renders** (no per-frame React state). Nested groups: outer = scroll (ScrollTrigger, `scroller`=snap-container, `scrub:1`); inner = idle (`sine.inOut` yoyo). Choreography: right cluster → King sweeps left / Queen right + scale + depth-match (flank "Trusted") → exit off-edges + fade before voice cards → reverses on scroll-up. `matchMedia` reduced-motion; `frameloop="never"` once exited. Perf: `dpr [1,1.4]`, 4 lights, cool rim so the black Queen reads. `HeroVideoFrames.tsx` (commentary stream) is **deferred** — removed from the hero for perf (`mix-blend-screen` over the full viewport was a per-frame GPU sink); component retained for later integration. |
