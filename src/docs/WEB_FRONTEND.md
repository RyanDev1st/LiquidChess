...
| Hero | `HeroSection.tsx` + `ChessHeroRig.tsx` | **Editorial split** (Editorial-Luxury / Split-Stage): serif copy + gold CTA on the LEFT, 3D pieces clustered RIGHT — no overlap. Charcoal `#0a0a0c` (never pure black), subtle warm radial glow, eval-bar tags. The R3F Canvas is a **viewport-fixed overlay `createPortal`'d to `<body>`** (NOT a snap-container child — being a child broke the snap hook's `children[i]=section i` index math and froze scrolling). Loads `chess-hero.glb` (named King/Queen/Mic). 3-tier groups: **root** = entrance rise + cursor parallax; **outer** = scroll (GSAP ScrollTrigger, `scroller`=snap-container, `scrub:0.8`); **inner** = organic multi-axis idle + proximity hover-glow. Scroll choreography is GSAP; continuous motion (idle + parallax + hover) lives in one damped `useFrame` inside `ChessHeroRig`. Interaction: canvas stays `pointer-events:none` (DOM clickable) while a `window` pointermove drives parallax + a cheap screen-space proximity test lifts/gold-glows the nearest piece (no per-frame mesh raycast). Materials upgraded to `MeshPhysicalMaterial` (clearcoat ceramic sheen). Choreography: right cluster → King sweeps left / Queen right + scale + depth-match (flank "Trusted") → exit off-edges + fade before voice cards → reverses on scroll-up. `matchMedia` reduced-motion (idle/parallax gated off); `frameloop="never"` once exited. Perf: `dpr [1,1.4]`, 4 lights, cool rim so the black Queen reads. `HeroVideoFrames.tsx` (commentary stream) is **deferred** — removed from the hero for perf (`mix-blend-screen` over the full viewport was a per-frame GPU sink); component retained for later integration. |
